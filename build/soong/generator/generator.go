...
func init() {
	android.RegisterModuleType("exthm_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = exthmExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = exthmExpandVariables(ctx, dep_file)
	...
	cmd := exthmExpandVariables(ctx, String(g.properties.Cmd))
